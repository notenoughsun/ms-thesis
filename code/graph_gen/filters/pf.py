...
class PF(LocalizationFilter):
    def __init__(self, initial_state, \
        alphas, betas, num_particles, update_w, field_map):
    ...
    def predict(self, u):
        ...
        for i in range(self.num_particles):
            self.x[i] = get_prediction(self.x[i], u)
    ...
    def update(self, z):
        ...
        #     return w
        self.w = self.update_w(self.field_map, self.x, z)
    ...
    def filter(self, data, initial_state):
        self.reinit(initial_state)
        u, z = data
        u = np.array(u).T
        z = np.array(z).T
        num_steps = u.shape[0]
